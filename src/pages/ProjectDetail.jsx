...
export default function ProjectDetail() {
  ...
  const allSkills = project.techBreakdown
    ? [...new Set(Object.values(project.techBreakdown).flat())]
    : []
      ...
      <div className="px-8 md:px-16 lg:px-20 py-16">
        ...
        <div className="hidden md:block">
          ...
          {project.showcaseImages?.length > 0 && (
            <div className={`mb-10 gap-4 ${project.showcaseImages.length > 1 ? 'grid grid-cols-1 sm:grid-cols-2' : 'flex flex-col'}`}>
                ...
                <div key={i} className="p-6 md:p-8" style={{ backgroundColor: 'rgba(255,255,255,0.75)' }}>
                  ...
                  {part.media?.length > 0 && (
                    <div className="mt-6 flex flex-col gap-6">
                      {part.media.map((item, k) => (
                        <div key={k}>
                          <div className="overflow-hidden rounded-sm" style={{ backgroundColor: `${color}10` }}>
                            <img src={item.src} alt={item.caption || `Demo ${k + 1}`} className="w-full h-auto block" />
                          </div>
                          {item.caption && (
                            <p className="text-xs text-muted mt-2 font-mono leading-relaxed">{item.caption}</p>
                          )}
                        </div>
                      ))}
                    </div>
          ...
          <div className="flex flex-wrap gap-3">
            {projects.filter(p => p.id !== id).slice(0, 3).map(p => (
              <Link key={p.id} to={`/project/${p.id}`}
                className="text-sm px-4 py-2 border border-border rounded-sm text-muted transition-colors"
